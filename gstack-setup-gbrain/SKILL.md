...
```bash
bun run ~/.hermes/skills/gstack/bin/gstack-memory-ingest.ts --probe
```
...
bulk via `bun run ~/.hermes/skills/gstack/bin/gstack-memory-ingest.ts --bulk --quiet`. Set
...
bun run ~/.hermes/skills/gstack/bin/gstack-gbrain-sync.ts --full --no-brain-sync
```
...
hook runs `bun run ~/.hermes/skills/gstack/bin/gstack-gbrain-sync.ts --incremental --quiet` on every skill
