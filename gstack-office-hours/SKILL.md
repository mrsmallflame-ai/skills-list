...
1. **Scan at sink first.** The repo copy leaves the private store, so scan the
   ...
   `~/.hermes/skills/gstack/bin/gstack-redact --from-file <tmp>`; exit 3
...
```bash
~/.hermes/skills/gstack/bin/gstack-config get founder_resources 2>/dev/null || echo "true"
```
...
`~/.hermes/skills/gstack/bin/gstack-config set founder_resources false`, then
