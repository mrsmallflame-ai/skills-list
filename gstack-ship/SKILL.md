...
```bash
~/.hermes/skills/gstack/bin/gstack-evidence run --label tests -- 'bin/test-lane 2>&1' &
~/.hermes/skills/gstack/bin/gstack-evidence run --label vitest -- 'npm run test 2>&1' &
wait
```
...
1. Read `~/.hermes/skills/gstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
> You are classifying Greptile review comments for a /ship workflow. Read `~/.hermes/skills/gstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
